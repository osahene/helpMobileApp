...
function createValidatorResult(
  ...
  const $message = isFunction$1(message)
    ? computed(() =>
        message(
          unwrapObj({
            $pending,
...
function collectNestedValidationResults(
  validations,
  nestedState,
  path,
  resultsCache,
...
function createMetaFields(results, nestedResults, childResults) {
  ...
  const $invalid = computed(
    () => allResults.value.some((r) => r.$invalid) || unref(results.$invalid) || false,
  )
  const $pending = computed(
    () => allResults.value.some((r) => unref(r.$pending)) || unref(results.$pending) || false,
  )
  ...
  checkRulesValidity(path, rules, storedRules) {
    ...
    return newRulesKeys.every((ruleKey) => {
      ...
      return Object.keys(rules[ruleKey].$params).every((paramKey) => {
        return (
          unref(storedRules[ruleKey].$params[paramKey]) === unref(rules[ruleKey].$params[paramKey])
        )
      })
    })
  }
...
function nestedValidations(_ref) {
  ...
  instance.__vuelidateInjectInstances = [].concat(
    instance.__vuelidateInjectInstances || [],
    injectChildResultsIntoParent,
  )
  function removeChildResultsFromParent(key) {
    childResultsKeys.value = childResultsKeys.value.filter((childKey) => childKey !== key)
    delete childResultsRaw[key]
  }
  instance.__vuelidateRemoveInstances = [].concat(
    instance.__vuelidateRemoveInstances || [],
    removeChildResultsFromParent,
  )
...
function ComputedProxyFactory(target) {
  return new Proxy(target, {
    get(target2, prop) {
      return typeof target2[prop] === 'object'
        ? ComputedProxyFactory(target2[prop])
        : computed(() => target2[prop])
    },
  })
}
let uid = 0
function useVuelidate(validations, state) {
  ...
  const { childResults, sendValidationResultsToParent, removeValidationResultsFromParent } =
    instance
      ? nestedValidations({
          $scope,
          instance,
  ...
  if (!validations && componentOptions.validations) {
    ...
    onBeforeMount(() => {
      ...
      watch(
        () =>
          isFunction$1(rules)
            ? rules.call(state.value, new ComputedProxyFactory(state.value))
            : rules,
        (validations2) => {
    ...
    const validationsWatchTarget =
      isRef(validations) || isProxy(validations) ? validations : reactive(validations || {})
    watch(
  ...
  return computed(() => {
    return _objectSpread2$1(
      _objectSpread2$1({}, unref(validationResults.value)),
      childResults.value,
    )
  })
}
...
function withParams($params, $validator) {
  if (!isObject($params))
    throw new Error(
      `[@vuelidate/validators]: First parameter to "withParams" should be an object, provided ${typeof $params}`,
    )
...
regex(/(^[0-9]*$)|(^-[0-9]+$)/)
regex(/^[-]?\d*(\.\d+)?$/)
export { common as c, email as e, required as r, sameAs as s, useVuelidate as u }
