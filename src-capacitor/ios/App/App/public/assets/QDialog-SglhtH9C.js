import {
  ...
  o as onMounted,
  a4 as nextTick,
  aE as getParentProxy,
  r as ref,
  b as onUnmounted,
  aB as injectProp,
  aF as Teleport,
  aG as createGlobalNode,
  aH as removeGlobalNode,
  aI as preventScroll,
  am as client,
  ai as isKeyCode,
  S as Transition,
...
import {
  r as removeFocusWaitFlag,
  b as addFocusWaitFlag,
  a as addFocusFn,
} from './focus-manager-B-vefqia.js'
...
const QDialog = createComponent({
  ...
  setup(props, { slots, emit, attrs }) {
    ...
    const classes = computed(
      () =>
        `q-dialog__inner flex no-pointer-events q-dialog__inner--${props.maximized === true ? 'maximized' : 'minimized'} q-dialog__inner--${props.position} ${positionClass[props.position]}` +
    ...
    function handleShow(evt) {
      addToHistory()
      refocusTarget =
        props.noRefocus === false && document.activeElement !== null ? document.activeElement : null
      ...
      registerTimeout(() => {
        if (vm.proxy.$q.platform.is.ios === true) {
          if (props.seamless !== true && document.activeElement) {
            const { top, bottom } = document.activeElement.getBoundingClientRect(),
              { innerHeight } = window,
