...
const QTabs = createComponent({
  ...
  setup(props, { slots, emit }) {
    ...
    const { registerTimeout: registerFocusTimeout, removeTimeout: removeFocusTimeout } =
      useTimeout()
    const { registerTimeout: registerScrollToTabTimeout, removeTimeout: removeScrollToTabTimeout } =
      useTimeout()
    const rootRef = ref(null)
    ...
    const alignClass = computed(() => {
      const align =
        scrollable.value === true ? 'left' : justify.value === true ? 'justify' : props.align
      return `q-tabs__content--align-${align}`
    })
    const classes = computed(
      () =>
        `q-tabs row no-wrap items-center q-tabs--${scrollable.value === true ? '' : 'not-'}scrollable q-tabs--${props.vertical === true ? 'vertical' : 'horizontal'} q-tabs__arrows--${props.outsideArrows === true ? 'outside' : 'inside'} q-tabs--mobile-with${props.mobileArrows === true ? '' : 'out'}-arrows` +
    ...
    function scrollToTabEl(el) {
      const { left, width, top, height } = contentRef.value.getBoundingClientRect(),
    ...
    function updateActiveRoute() {
      ...
      if (
        name === null &&
        tabDataList.some(
          (tab) => tab.routeData === void 0 && tab.name.value === currentModel.value,
        ) === true
      ) {
        hadActivated = false
        return
