...
function useCheckbox(type, getInner) {
  ...
  const innerClass = computed(() => {
    ...
    const color =
      props.color !== void 0 && (props.keepColor === true || isFalse.value !== true)
        ? ` text-${props.color}`
        : ''
  ...
  const attributes = computed(() => {
    const attrs = {
      ...
      'aria-checked':
        isIndeterminate.value === true ? 'mixed' : isTrue.value === true ? 'true' : 'false',
    }
    if (props.disable === true) {
  ...
  return () => {
    ...
    const label =
      props.label !== void 0 ? hMergeSlot(slots.default, [props.label]) : hSlot(slots.default)
    label !== void 0 &&
      child.push(
        h(
          'div',
