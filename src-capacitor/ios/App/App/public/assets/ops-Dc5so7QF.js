...
const useOperations = defineStore("ops", {
  state: () => ({
    myDependants: [],
    myContacts: []
  }),
  actions: {
    ...
    async alertTrigger(data) {
      try {
        console.log("Triggering alert with data:", data);
        ...
        Notify.create({
          type: "negative",
          message: error.response.message || error.response.data.message || "Failed triggering alert"
        });
      }
    }
  }
